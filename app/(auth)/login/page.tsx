...
const LoginSchema = Yup.object({
  identifier: Yup.string().required("رقم الجوال او الايميل مطلوب"),
  password: Yup.string()
            ...
            {({ errors, touched, isSubmitting }) => (
              <Form className="space-y-8">
                ...
                <div className="flex flex-col gap-2">
                  <label htmlFor="identifier" className="text-[#171D5B] font-bold text-sm pr-1">
                    رقم الجوال او الايميل
                  </label>
                  <Field 
                    id="identifier"
                    name="identifier"
                    ...
                    className={`w-full bg-[#F8F9FA] rounded-xl py-4 px-6 text-brand-blue font-bold focus:outline-none focus:ring-2 transition-all 
                      ${errors.identifier && touched.identifier ? 'ring-2 ring-red-500' : 'focus:ring-brand-gold'}`}
                  />
                  <ErrorMessage name="identifier" component="span" className="text-red-500 text-xs font-bold pr-1" />
                </div>

                {/* Password Input */}
                <div className="flex flex-col gap-2">
                  <label htmlFor="password" className="text-[#171D5B] font-bold text-sm pr-1">
                    كلمة المرور
                  </label>
                  <Field 
                    id="password"
                    ...
                    className={`w-full bg-[#F8F9FA] rounded-xl py-4 px-6 text-brand-blue font-bold focus:outline-none focus:ring-2 transition-all 
                      ${errors.password && touched.password ? 'ring-2 ring-red-500' : 'focus:ring-brand-gold'}`}
                  />
                  <ErrorMessage name="password" component="span" className="text-red-500 text-xs font-bold pr-1" />
                  
                  <div className="flex justify-start mt-1">
                    <Link href="/forgot-password" className="text-blue-500 text-xs font-bold hover:underline transition-colors">
                       نسيت كلمة المرور
                    </Link>
                  </div>
                </div>

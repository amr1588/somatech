      ...
      <div className="relative z-10 w-full max-w-[1200px] flex flex-col items-center py-12 px-6">
        ...
        <div className="w-full max-w-[500px] bg-white rounded-md p-10 md:p-12 shadow-2xl">
          <div className="text-center mb-10">
            <h1 className="text-3xl font-black text-brand-blue mb-3">تأكيد كلمة المرور</h1>
            <p className="text-gray-400 text-sm font-bold leading-relaxed px-4">
              برجاء تأكيد كلمة المرور الجديدة
            </p>
          </div>
            ...
            {({ errors, touched, isSubmitting }) => (
              <Form className="space-y-8">
                ...
                <div className="flex flex-col gap-2">
                  <label className="text-[#171D5B] font-bold text-sm pr-1">كلمة المرور</label>
                  <Field 
                    name="password"
                    type="password"
                    placeholder="ادخل كلمة المرور الجديدة"
                    className={`w-full bg-[#F8F9FA] rounded-xl py-4 px-6 text-brand-blue font-bold placeholder:text-gray-300 focus:outline-none focus:ring-2 transition-all ${errors.password && touched.password ? 'ring-red-500' : 'focus:ring-brand-gold'}`}
                  />
                  <ErrorMessage name="password" component="div" className="text-red-500 text-xs font-bold pr-1" />
                </div>

                {/* Confirm Password Input */}
                <div className="flex flex-col gap-2">
                  <label className="text-[#171D5B] font-bold text-sm pr-1">تأكيد كلمة المرور</label>
                  <Field 
                    name="confirmPassword"
                    type="password"
                    placeholder="تأكيد كلمة المرور الجديدة"
                    className={`w-full bg-[#F8F9FA] rounded-xl py-4 px-6 text-brand-blue font-bold placeholder:text-gray-300 focus:outline-none focus:ring-2 transition-all ${errors.confirmPassword && touched.confirmPassword ? 'ring-red-500' : 'focus:ring-brand-gold'}`}
                  />
                  <ErrorMessage name="confirmPassword" component="div" className="text-red-500 text-xs font-bold pr-1" />
                </div>

...
import AwardingModal from "./AwardingModal";
...
export default function ChairmanView() {
  ...
  const [openingPrices, setOpeningPrices] = useState<string[]>(Array(12).fill(""));
  const [diffPrices, setDiffPrices] = useState<string[]>(Array(12).fill(""));
  const [hedgingValues, setHedgingValues] = useState<string[]>(Array(12).fill(""));
    ...
    return (
      <div className="flex-1 flex flex-col min-h-0 overflow-hidden" dir="rtl">
        ...
        <div className="mb-2 flex justify-between items-center shrink-0">
          ...
          <button className="bg-orange-400 text-white px-8 py-3 rounded-xl text-[14px] font-black hover:bg-orange-500 transition-all shadow-sm me-16">
            بيان المزاد
          </button>
        </div>

        {/* Chairman Detail Table */}
        <div className="flex-1 min-h-0 overflow-auto bg-[#F8F9FB] rounded-[24px] px-5 pb-5 scrollbar-hide border border-gray-100/50">
          <table className="w-full border-separate border-spacing-y-2">
            <thead className="sticky top-0 z-10">
              <tr className="text-right text-gray-400 text-[11px] font-bold uppercase">
                ...
                <th className="pt-5 pb-3 text-center sticky top-0 bg-[#F8F9FB] z-10">سعر الافتتاح</th>
                <th className="pt-5 pb-3 text-center sticky top-0 bg-[#F8F9FB] z-10">سعر فرق السوم</th>
                <th className="pt-5 pb-3 text-center sticky top-0 bg-[#F8F9FB] z-10">قيمة التحوط</th>
                <th className="pt-5 pb-3 text-center sticky top-0 bg-[#F8F9FB] z-10">حفظ الادخال</th>
            ...
            <tbody>
              {[...Array(12)].map((_, idx) => (
                <ChairmanTableRow 
                  key={idx} 
                  idx={idx} 
                  assetImage={idx % 2 === 0 ? "/mazayaGarden.jpg" : "/mazayaTower.jpg"}
                  setAssetModal={(title, img) => {
                    setSelectedAssetTitle(title);
                    setSelectedAssetImage(img);
                    setIsAssetModalOpen(true);
                  }}
                  setAwardingModal={(title, img) => {
                    setSelectedAssetTitle(title);
                    setSelectedAssetImage(img);
                    setIsAwardingModalOpen(true);
                    setActiveRowIdx(idx);
                  }}
                  awardingStatus={awardingStatuses[idx]}
                  openingPrice={openingPrices[idx]}
                  diffPrice={diffPrices[idx]}
                  hedgingValue={hedgingValues[idx]}
                  onInputChange={(field, val) => {
                    if (field === "opening") {
                      const newPrices = [...openingPrices];
                      newPrices[idx] = val;
                      setOpeningPrices(newPrices);
                    } else if (field === "diff") {
                      const newPrices = [...diffPrices];
                      newPrices[idx] = val;
                      setDiffPrices(newPrices);
                    } else if (field === "hedging") {
                      const newValues = [...hedgingValues];
                      newValues[idx] = val;
                      setHedgingValues(newValues);
                    }
                  }}
        ...
        <AwardingModal
          isOpen={isAwardingModalOpen}
          onClose={() => setIsAwardingModalOpen(false)}
          assetTitle={selectedAssetTitle}
          assetImage={selectedAssetImage}
          initialOpeningPrice={activeRowIdx !== null ? openingPrices[activeRowIdx] : ""}
          initialDiffPrice={activeRowIdx !== null ? diffPrices[activeRowIdx] : ""}
          initialHedgingValue={activeRowIdx !== null ? hedgingValues[activeRowIdx] : ""}
          onAward={() => {
          ...
          onSave={(opening, diff, hedging) => {
            if (activeRowIdx !== null) {
              const newOpening = [...openingPrices];
              newOpening[activeRowIdx] = opening;
              setOpeningPrices(newOpening);

              const newDiff = [...diffPrices];
              newDiff[activeRowIdx] = diff;
              setDiffPrices(newDiff);

              const newHedging = [...hedgingValues];
              newHedging[activeRowIdx] = hedging;
              setHedgingValues(newHedging);
      ...
      <div className="flex items-center justify-between mb-1 shrink-0 px-2" dir="ltr">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="text-[14px] font-bold text-gray-800" dir="rtl">
            رئيس اللجنة <span className="text-orange-500">(53)</span>
          </div>
        </div>
      </div>
...
function ChairmanTableRow({ 
  ...
  idx: number; 
  assetImage: string; 
  setAssetModal: (title: string, img: string) => void; 
  setAwardingModal: (title: string, img: string) => void; 
  awardingStatus: null | "awarded" | "rejected"; 
  openingPrice: string;
  diffPrice: string;
  hedgingValue: string;
  onInputChange: (field: "opening" | "diff" | "hedging", val: string) => void;
  setActiveRowIdx: (idx: number) => void;
  getStatusBadge: (status: any) => React.ReactNode 
}) {
  const [isSaved, setIsSaved] = useState(false);

  const isInputEmpty = !openingPrice || !diffPrice || !hedgingValue;
    ...
    <tr className="hover:bg-white transition-all group">
      <td className="py-2.5 px-6 bg-white rounded-r-2xl border-y border-r border-gray-50/50 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-[12px] font-bold text-gray-800">#30944266</div>
      ...
      <td className="py-2.5 text-center bg-white border-y border-gray-50/50 shadow-sm">
        <div className="flex justify-center transform scale-90">{getStatusBadge(idx < 6 ? "approved" : idx < 9 ? "ended" : "rejected")}</div>
      </td>
      <td className="py-2.5 text-center bg-white border-y border-gray-50/50 shadow-sm text-[12px] font-bold text-gray-800 whitespace-nowrap">
        86.742.62 <span className="text-[10px]">ر.س</span>
      </td>
      <td className="py-2.5 text-center bg-white border-y border-gray-50/50 shadow-sm px-2">
        <div className="flex justify-center items-center">
          {awardingStatus === "awarded" ? (
            <button className="flex items-center gap-1 px-3 py-1.5 bg-orange-50 text-orange-500 rounded-lg border border-orange-100 text-[12px] font-bold hover:bg-orange-100 transition-all min-w-[120px]">
              <Gavel size={12} className="me-1" />
              تم الترسية
            </button>
          ) : awardingStatus === "rejected" ? (
            <button className="flex items-center gap-1 px-3 py-1.5 bg-rose-50 text-rose-500 rounded-lg border border-rose-100 text-[12px] font-bold hover:bg-rose-100 transition-all min-w-[120px]">
              <XCircle size={12} />
              رفض الترسية
            </button>
          ) : (
            <div 
              ...
              onClick={(e) => {
                e.stopPropagation();
                setAwardingModal(title, assetImage);
                setActiveRowIdx(idx);
              }}
            >
      ...
      <td className="py-2.5 text-center bg-white border-y border-gray-50/50 shadow-sm">
        <div className="px-2">
          <input 
            type="text" 
            value={openingPrice}
            onChange={(e) => onInputChange("opening", e.target.value)}
            disabled={isSaved}
            className={`w-full bg-[#F8F9FB] border border-gray-100 rounded-lg py-1.5 px-2 text-[12px] font-bold text-gray-700 text-center outline-none focus:border-blue-200 transition-all ${isSaved ? 'opacity-70 bg-gray-50' : ''}`}
          />
        </div>
      </td>
      <td className="py-2.5 text-center bg-white border-y border-gray-50/50 shadow-sm">
        <div className="px-2">
          <input 
            type="text" 
            value={diffPrice}
            onChange={(e) => onInputChange("diff", e.target.value)}
            disabled={isSaved}
            className={`w-full bg-[#F8F9FB] border border-gray-100 rounded-lg py-1.5 px-2 text-[12px] font-bold text-gray-700 text-center outline-none focus:border-blue-200 transition-all ${isSaved ? 'opacity-70 bg-gray-50' : ''}`}
          />
        </div>
      ...
      <td className="py-2.5 text-center bg-white border-y border-gray-50/50 shadow-sm">
        <div className="px-2">
          <input 
            type="text" 
            value={hedgingValue}
            onChange={(e) => onInputChange("hedging", e.target.value)}
            disabled={isSaved}
            className={`w-full bg-[#F8F9FB] border border-gray-100 rounded-lg py-1.5 px-2 text-[12px] font-bold text-gray-700 text-center outline-none focus:border-blue-200 transition-all ${isSaved ? 'opacity-70 bg-gray-50' : ''}`}
          />
        </div>
      ...
      <td className="py-2.5 text-center bg-white border-y border-gray-50/50 shadow-sm">
        <div className="flex justify-center">
          <button 
            onClick={() => !isInputEmpty && setIsSaved(true)}
            disabled={isSaved || isInputEmpty}
            className={`flex items-center justify-center gap-1.5 px-4 py-1.5 rounded-lg text-[11px] font-bold transition-all min-w-[105px] ${
              isSaved 
                ? "bg-[#00A962] text-white" 
                : isInputEmpty 
                  ? "bg-gray-300 text-white cursor-not-allowed" 
                  : "bg-[#4CAF50] text-white hover:bg-green-600 active:scale-95 cursor-pointer shadow-sm"
            }`}
          >
